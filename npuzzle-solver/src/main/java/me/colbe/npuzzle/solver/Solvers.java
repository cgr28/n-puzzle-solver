...
public class Solvers {
    ...
    public PriorityQueue<State> potentialStates(State state) {
        ...
        for (Move move : new Move[] { Move.DOWN, Move.LEFT, Move.RIGHT, Move.UP }) {
            ...
            if (tempPuzzle.moveTile(move)) {
                tempState = new State(parent.getG() + 1, searchHeuristic.calculateHeuristic(tempPuzzle), tempPuzzle,
                        parent, move);
    ...
    public idaStarReturn idaStarSearch(State state, int thresh, Heuristic heuristic, HashMap<Integer, int[]> goalMap,
            HashMap<String, Integer> pdb) {
